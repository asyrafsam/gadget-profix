        ...
        <ul class="nav navbar-nav navbar-right">
          <li class="dropdown">
            ...
            <ul class="dropdown-menu" role="menu">
              <li><a href="<?php echo base_url(). 'func_excel/excel_client/'.$random; ?>">&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-fw fa-file-excel"></i> <span class="badge">EXCEL</a></li>
              <li><a href="<?php echo base_url(). 'func_pdf/pdf_client/'.$random; ?>">&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-fw fa-file-pdf"></i> <span class="badge">PDF</a></li>
                <li><a href="#">&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-fw fa-users"></i> <span class="badge">Add Client by CSV</a></li>
            </ul>
          </li>
        </ul>
      </div>
    ...
    <div class="modal fade" id="viewClientModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-custom-2" role="document">
        <div class="modal-content">
          ...
          <form action="" method="post" id="formXY" enctype="multipart/form-data" >
            <div class="modal-body">
              <div class="flex-reparation col-lg-12">
                <div class="flex-child col-lg-6">
                  <div class="flex-row">
                    <input type="hidden" name="cid-3" id="cid-3" class="form-control">
                    <div class="col-lg-12">
                      <label>Name</label>
                    </div>
                    <div class="form-group input-group col-lg-12">
              ...
              <div class="flex-reparation col-lg-12">
                <div class="form-group input-group col-lg-12" style="height: 200px;">
                  <div class="table-responsive">
                    <hr>
                    <table class="table table-striped table-bordered" id="dataTable1" width="100%" cellspacing="0" style="font-size: 13px;">
                      <thead>
                        <tr>
  ...
  $(document).ready(function() {
    $('#dataTable').DataTable( {
      ...
      function getDetailClient(id)
        ...
        //$(document).ready(function() {
        $("[name='checkclient']").click(function() {
            ...
            if (checked) {
                ...
                var data = {'id':id}
                $.ajax({
                      url: '<?= base_url() ?>func_client/getDetailsClient',
                      type: 'POST',
                      dataType: 'json',
                      data: data,
                      beforeSend: function() {
                      ...
                      success: function(response){
                        ...
                        var c_id = response.c_id;
                        var cl_name = response.c_name;
                        var geolocate = response.c_geolocate;

                        var hold_value = $("#hold_value").val();

                        storeClient(c_id,hold_value); // add 
                      }
              });
        ...
        function deletePrint(id)
        {
          ...
          $.ajax({
                          url: '<?= base_url() ?>func_client/deletePrint',
                          type: 'POST',
                          dataType: 'html',
                          data: data,
                          beforeSend: function() {
        ...
        function storeClient(c_id,hold_value){
          var data = {'c_id':c_id,'hold_value':hold_value}
          $.ajax({
                          url: '<?= base_url() ?>func_client/storeClient',
                          type: 'POST',
                          dataType: 'html',
                          data: data,
                          beforeSend: function() {
      ...
      function view_client(id){
          ...
          /*load data dari ajax sama macam concept atas*/
          $.ajax({
            url:'<?= base_url() ?>func_client/viewClient',
            type: "POST",
            dataType: 'html',
            data: data,
            success: function(data){
      ...
      function viewClientJson(id)
        {
          var data = {'id':id}
          $.ajax({
                          url: '<?= base_url() ?>func_client/viewClientJson',
                          type: 'POST',
                          dataType: 'json',
                          data: data,
                          beforeSend: function() {
                          ...
                          success: function(data){
                            ...
                            $('[name="cid-3"]').val(data.c_id);
                            $('[name="cname-3"]').val(data.c_name);
                            $('[name="clocate-3"]').val(data.c_geolocate);
                            $('[name="ccity-3"]').val(data.c_city);
                            $('[name="ctelephone-3"]').val(data.c_telephone);
                            $('[name="cvat-3"]').val(data.c_vat);
                            $('[name="ccompany-3"]').val(data.c_company);
                            $('[name="caddress-3"]').val(data.c_address);
                            $('[name="cpostal-3"]').val(data.c_postal);
                            $('[name="cemail-3"]').val(data.c_email);
                            $('[name="cssn-3"]').val(data.c_ssn);
                            $('[name="ccomment-3"]').val(data.c_comment);
      ...
      function view_clientRepair(id){
          ...
          /*load data dari ajax sama macam concept atas*/
          $.ajax({
            url:'<?= base_url() ?>func_client/viewClientRepair',
            type: "POST",
            dataType: 'html',
            data: data,
            success: function(data){
              /*cara nak tarik value form data menggunakan name*/
              $('#dataTable-1 tbody').html(data);
              /*ajax panggil modal dan text*/
