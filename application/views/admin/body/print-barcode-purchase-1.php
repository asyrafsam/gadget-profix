...
<div class="container-fluid">
  <h1 class="h3 mb-2 text-gray-800">Print Barcode</h1>
  <hr>
            ...
            <div style="top:10px;">
              <h6 style="font-weight: bold;color: black;">Order Items</h6>
               <table class="table table-striped table-bordered" cellspacing="0" style="font-size: 13px;width: 152%;">
                ...
                <tbody >
                  <?php foreach ($item as $idid) {
                    ?>
                  <tr>
                    <td><?php echo $idid->itemName?></td>
                    <td><input type="text" name="exists" id="exists" value="<?php echo $idid->itemPrice?>"></td>
                    <td><?php echo $idid->itemQuantity?></td>
                    <td>0.00</td>
                    <td></td>
                    <td><a onclick="deleteholdpurchase(<?php echo $idid->itemID;?>,<?php echo $idid->hold_id;?>)"><button type="button">x</button></a></td>
                  </tr>
                  <?php } ?>
                  <tfoot id="dataList">
...
<script>
  ...
  function getTotal()
  {
    ...
    $.ajax({
                    ...
                    success: function(response){
                      var totalall = parseInt(response) + parseInt(existing);
                      $("#total_subtotal").val(parseInt(totalall));
  ...
  function getDetailHold(hold_value)
  {
    var data = {'hold_value':hold_value}
    $.ajax({
                    url: '<?= base_url() ?>func_purchase/getDetailsBarcode',
                    type: 'POST',
                    dataType: 'html',
                    data: data,
                    beforeSend: function() {
  ...
  function deleteholdpurchase(id,hold_id){
  // if(confirm('Are you sure?')) {
      var data = {'id':id,'holdid':hold_id}
    $.ajax({
                    url: '<?= base_url() ?>func_purchase/deleteholdpurchase',
                    type: 'POST',
                    dataType: 'html',
                    data: data,
                    beforeSend: function() {
